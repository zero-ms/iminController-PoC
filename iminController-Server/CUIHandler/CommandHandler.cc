...
int CommandHandler::executeCommand() {
  if (SplittedCommand[0] == "list" && SplittedCommand.size() == 1) {
    int i = 1;
    for (auto iter = BotHandler::getBotListIteratorBegin();
         iter != BotHandler::getBotListIteratorEnd(); iter++) {
    ...
    if (SplittedCommand.size() >= 4 && SplittedCommand.size() <= 8) {
      ...
      if (SplittedCommand[1] == "remove") {
        TaskHandler::removeTask(std::stoi(SplittedCommand[2]));
      ...
      if (SplittedCommand[1] == "list") {
        for (auto iter = TaskHandler::getTaskPoolIteratorBegin();
             iter != TaskHandler::getTaskPoolIteratorEnd(); iter++) {
