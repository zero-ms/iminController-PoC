...
#define ALLOC_ON_CODE __attribute__((section(".text")))
#else
#define ALLOC_ON_CODE                                                          \
  _Pragma("section(\".text\")") __declspec(allocate(".text"))
#endif
...
void freshycalls::Syscall::ExtractStubs() noexcept {
  ...
  const auto ntdll_ldr_entry = reinterpret_cast<native::LdrDataEntry *>(
      peb->Ldr->InLoadOrderModuleList.Flink->Flink);
  ...
  const auto nt_headers = reinterpret_cast<native::NTHeaders64 *>(
      ntdll_base + dos_header->e_lfanew);

  const auto export_dir = reinterpret_cast<native::ExportDirectory *>(
      ntdll_base +
      nt_headers->OptionalHeader.DataDirectory[native::kExport].VirtualAddress);

  const auto functions_table =
      reinterpret_cast<uint32_t *>(ntdll_base + export_dir->AddressOfFunctions);
  const auto names_table =
      reinterpret_cast<uint32_t *>(ntdll_base + export_dir->AddressOfNames);
  const auto names_ordinals_table = reinterpret_cast<uint16_t *>(
      ntdll_base + export_dir->AddressOfNameOrdinals);
  ...
  for (size_t i = 0; i < export_dir->NumberOfNames; i++) {
    ...
    if (function_name.rfind("Nt", 0) == 0 &&
        function_name.rfind("Ntdll", 0) == std::string::npos &&
        function_name.compare("NtGetTickCount")) {
      stub_ordinal = names_ordinals_table[i];
      stub_address = ntdll_base + functions_table[stub_ordinal];

      stub_map.insert({stub_address, djb2((PBYTE)function_name.c_str())});
      int a = 3;
    }
  }
}

[[nodiscard]] uintptr_t freshycalls::Syscall::GetStubAddr(DWORD64 stub_name) {
...
[[nodiscard]] uint32_t
freshycalls::Syscall::GetSyscallNumber(DWORD64 stub_name) {
  const auto syscall_entry = syscall_map.find(stub_name);
